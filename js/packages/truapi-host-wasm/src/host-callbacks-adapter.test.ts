...
describe("createWasmRawCallbacks", () => {
    ...
    it("bridges lifecycle, confirmations, and preimage callbacks", async () => {
        ...
        expect(await raw.submitPreimage!(new Uint8Array([6]))).toEqual(new Uint8Array([7, 8, 9]));
        ...
        expect(calls).toEqual([
            ["lookupPreimage", [9]],
            ["authStateChanged", { tag: "Pairing", value: { deeplink: "polkadotapp://example" } }],
    ...
    it("bridges typed chain connections", async () => {
        ...
        const connection = await raw.chainConnect!(GENESIS, (json) => received.push(json));
