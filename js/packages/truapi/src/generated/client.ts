...
export class TrUApiCallsClient {
  ...
  async pushNotification(
    request: T.NotificationsHostPushNotificationRequest,
  ): Promise<
    Result<
      T.NotificationsHostPushNotificationResponse,
      T.NotificationsHostPushNotificationError
    >
  > {
    const result = await this.transport.request<
      S.ResultPayload<
   ...
   */
  async pushNotificationCancel(
    request: T.NotificationsHostPushNotificationCancelRequest,
  ): Promise<Result<undefined, T.GenericError>> {
    const result = await this.transport.request<
      S.ResultPayload<undefined, T.GenericError>
    >({
      ...
      decodeResponse: (payload) =>
        S.indexedTaggedUnion({
          V2: [1, S.Result(S._void, T.GenericError)] as const,
        }).dec(payload).value,
    });
    return result.success ? ok(result.value) : err(result.value);
  }
