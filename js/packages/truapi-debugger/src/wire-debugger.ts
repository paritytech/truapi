...
export interface WireTrace {
  ...
  channelId: string;
  /** Correlation id shared by every frame in this trace (unique within the channel). */
  requestId: string;
...
export interface WireDebugger {
  ...
  traces(): WireTrace[];
  /**
   * The trace for a specific `requestId`. Pass `channelId` to disambiguate when
   * more than one host is connected (each mints the same `p:N` ids); without it,
   * the first trace matching `requestId` in activity order is returned - fine for
   * a single-host session or product-span (`correlationId`) correlation.
   */
  trace(requestId: string, channelId?: string): WireTrace | undefined;
...
export function createWireDebugger(options: WireDebuggerOptions = {}): WireDebugger {
  ...
  const observe: TransportObserver = (frame) => {
    const key = keyOf(frame.channelId, frame.requestId);
    ...
    if (trace.frames.length > maxFramesPerTrace) {
      // Evict oldest to keep an exact hard cap. This is O(cap) per frame once
      // the cap is reached; kept deliberately simple over an O(1) ring buffer
  ...
  return {
    ...
    trace: (requestId, channelId) => {
      if (channelId !== undefined) return traces.get(keyOf(channelId, requestId));
      // No channel given: first trace matching this requestId in activity order.
