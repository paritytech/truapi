...
The Dart client is generated by `truapi-codegen --dart-output` (see
`rust/crates/truapi-codegen/src/dart.rs`). Golden cross-language wire vectors are
produced by `cargo run -p truapi --example wire_vectors`. Run `make dart` to
regenerate, refresh vectors, analyze, and test. Design notes:
