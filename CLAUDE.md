...
rust/crates/
  ...
  truapi-codegen/        rustdoc JSON → TypeScript client + Dart client + Rust dispatcher
...
dart/
  truapi/         Dart/Flutter client; generated Dart under lib/src/generated (ignored)
playground/              Next.js interactive playground; deploys to truapi-playground.dot
hosts/dotli/             dotli submodule
docs/                    design docs, RFCs, feature proposals
scripts/codegen.sh       regenerate the TS + Dart clients from the Rust crate
```

The Dart client (`--dart-output`) and host dispatcher (`--dart-host-output`) are
both generated by `truapi-codegen` (see `rust/crates/truapi-codegen/src/dart.rs`).
The client is `package:truapi/truapi.dart`; the host is `package:truapi/host.dart`
(implement `TruapiHostHandlers`, wire with `createTruapiServer`). Golden
cross-language wire vectors are produced by `cargo run -p truapi --example
wire_vectors`. Run `make dart` to regenerate, refresh vectors, analyze, and test.
Design notes: `docs/design/dart-flutter-support.md`.
