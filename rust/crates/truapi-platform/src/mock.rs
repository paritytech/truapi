...
impl ProductStorage for MockPlatform {
    async fn read(
        &self,
        key: String,
    ) -> Result<Option<Vec<u8>>, latest::HostLocalStorageReadError> {
        if let Some(reason) = &self.config.faults.storage_error {
            return Err(latest::HostLocalStorageReadError::Unknown {
                reason: reason.clone(),
            });
        }
        Ok(self
    ...
    async fn write(
        ...
        key: String,
        value: Vec<u8>,
    ) -> Result<(), latest::HostLocalStorageReadError> {
        if let Some(reason) = &self.config.faults.storage_error {
            return Err(latest::HostLocalStorageReadError::Unknown {
                reason: reason.clone(),
            });
    ...
    async fn clear(&self, key: String) -> Result<(), latest::HostLocalStorageReadError> {
        if let Some(reason) = &self.config.faults.storage_error {
            return Err(latest::HostLocalStorageReadError::Unknown {
                reason: reason.clone(),
            });
...
impl ThemeHost for MockPlatform {
    fn subscribe_theme(
        &self,
    ) -> BoxStream<'static, Result<latest::ThemeVariant, latest::GenericError>> {
        let theme = self.config.theme;
        // Emit the current theme, then stay open (a live subscription never
        // ends), matching the real host contract.
        Box::pin(
            stream::once(async move { Ok::<latest::ThemeVariant, latest::GenericError>(theme) })
                .chain(stream::pending::<
                    Result<latest::ThemeVariant, latest::GenericError>,
                >()),
        )
    }
}
...
mod tests {
    ...
    fn resource_review() -> UserConfirmationReview {
        UserConfirmationReview::ResourceAllocation(latest::HostRequestResourceAllocationRequest {
            resources: vec![],
        })
    }
