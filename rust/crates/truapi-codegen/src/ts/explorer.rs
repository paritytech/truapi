...
pub fn generate_explorer_registry(
    ...
    fs::write(codegen_dir.join("registry.ts"), code)?;
...
fn generate_explorer_registry_code(api: &ApiDefinition, target_version: u32) -> Result<String> {
    ...
    for version in 1..=target_version {
        ...
        let minor = protocol_minor(version);
        writedoc!(
            out,
            r#"
              {{
                ...
                status: "stable",
            "#,
            id = ts_string_literal(&minor),
            label = ts_string_literal(&format!("v{minor}")),
            slug = ts_string_literal(&minor),
        )
        .unwrap();
...
fn explorer_methods(
    ...
    for service in explorer_traits(api)? {
        let trait_def = service.trait_def;
...
fn explorer_traits(api: &ApiDefinition) -> Result<Vec<PublicService<'_>>> {
    public_services(api)
}
...
fn write_explorer_groups_array(out: &mut String, indent: &str, groups: &[ExplorerGroupRecord]) {
    ...
    for group in groups {
        writedoc!(
            ...
            {indent}    name: {name},
            ",
            id = ts_string_literal(&group.id),
            name = ts_string_literal(&group.name),
        )
        .unwrap();
...
fn write_explorer_methods_array(out: &mut String, indent: &str, methods: &[ExplorerMethodRecord]) {
    ...
    for method in methods {
        writedoc!(
            ...
            {indent}    groupName: {group_name},
            {indent}    wireId: {wire_id},
            {indent}    pattern: {pattern},
            {indent}    request: {request},
            {indent}    response: {response},
            ",
            id = ts_string_literal(&method.id),
            name = ts_string_literal(&method.name),
            group_id = ts_string_literal(&method.group_id),
            group_name = ts_string_literal(&method.group_name),
            wire_id = method.wire_id,
            pattern = ts_string_literal(method.pattern),
            request = ts_string_literal(&method.request),
            response = ts_string_literal(&method.response),
        )
        .unwrap();
...
fn write_explorer_types_array(out: &mut String, indent: &str, types: &[ExplorerTypeRecord]) {
    ...
    for ty in types {
        writedoc!(
            ...
            ",
            id = ts_string_literal(&ty.id),
            name = ts_string_literal(&ty.name),
            category = ts_string_literal(&ty.category),
            definition = ts_string_literal(&ty.definition),
        )
        .unwrap();
        ...
        if !ty.fields.is_empty() {
            ...
            for field in &ty.fields {
                writedoc!(
                    ...
                    {indent}        type: {ty},
                    ",
                    name = ts_string_literal(&field.name),
                    ty = ts_string_literal(&field.ty),
                )
                .unwrap();
        ...
        if !ty.variants.is_empty() {
            ...
            for variant in &ty.variants {
                writedoc!(
                    ...
                    {indent}        type: {ty},
                    ",
                    name = ts_string_literal(&variant.name),
                    ty = ts_string_literal(&variant.ty),
                )
                .unwrap();
