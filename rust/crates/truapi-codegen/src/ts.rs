    ...
    match (&method.kind, &method.return_type) {
        (MethodKind::Request, ReturnType::Result { ok, err }) => {
            ...
            writedoc!(
                out,
                "
                  async {ts_method_name}({arg_decl}): Promise<Result<{ok_type}, {err_type}>> {{
                    const result = await this.transport.request<
                      S.ResultPayload<{ok_type}, {err_type}>
                    >({{
                      ids: W.{wire_const},
                ",
...
pub fn generate_host(api: &ApiDefinition, output_dir: &str, target_version: u32) -> Result<()> {
    ...
    fs::write(
        Path::new(output_dir).join("index.ts"),
...
fn emit_host_handler_signature(
    ...
    let request_arg = if method.kind == MethodKind::Request
        && trait_def.name == "System"
        && method.name == "handshake"
    {
        // The handshake's only field is `codecVersion` which the transport
        // provides automatically, but the host still receives the decoded
        // request as input for consistency.
        format!("request: {}, ", payload.inner_type_ts)
    } else if payload.param_list.is_empty() {
        String::new()
    } else {
        format!("request: {}, ", payload.inner_type_ts)
    };
    ...
    match (&method.kind, &method.return_type) {
        (MethodKind::Request, ReturnType::Result { ok, err }) => {
            ...
            writeln!(
                out,
                "  {ts_method_name}({request_arg}ctx: CallContext): Promise<Result<{ok_type}, {err_type}>>;",
                ok_type = response.inner_type_ts,
                err_type = error.inner_type_ts,
            )
            .unwrap();
        ...
        (MethodKind::Subscription, ReturnType::Subscription(ty)) => {
            ...
            writeln!(
                out,
                "  {ts_method_name}({request_arg}sink: SubscriptionSink<{item_type}>, ctx: CallContext): Promise<SubscriptionCleanup> | SubscriptionCleanup;",
                item_type = response.inner_type_ts,
            )
            .unwrap();
        }
        (MethodKind::ResultSubscription, ReturnType::ResultSubscription { item, err }) => {
            ...
            writeln!(
                out,
                "  {ts_method_name}({request_arg}sink: SubscriptionSink<{item_type}, {reason_type}>, ctx: CallContext): Promise<SubscriptionCleanup> | SubscriptionCleanup;",
                item_type = response.inner_type_ts,
                reason_type = error.inner_type_ts,
            )
            .unwrap();
        }
        (kind, return_type) => {
...
fn emit_host_entry(
    ...
    let wire_version = method_wire_version(method, wrappers, target_version)?;
    let payload = emit_payload(&method.params, wrappers, ctx, wire_version)?;
    let request_decode_expr = host_request_decode_expr(&payload, wire_version);
    let has_request_param = !payload.param_list.is_empty()
        || (trait_def.name == "System" && method.name == "handshake");
    let request_arg = if has_request_param { "request, " } else { "" };
    ...
    match (&method.kind, &method.return_type) {
        (MethodKind::Request, ReturnType::Result { ok, err }) => {
            ...
            let wire_arg = match wire_version {
                Some(version) => {
                    format!("{{ tag: 'V{version}' as const, value: payload }}")
                }
                ...
                "
                    {{
                      ...
                      async handle(bytes, ctx) {{
                        {request_decode_expr}
                        const result = await handlers.{ts_method_name}({request_arg}ctx);
                        const payload = result.isOk()
                          ? {{ success: true as const, value: result.value }}
                          : {{ success: false as const, value: result.error }};
                        return {response_codec}.enc({wire_arg});
                      }},
                    }},
                "
            )
        ...
        (MethodKind::Subscription, ReturnType::Subscription(ty)) => {
            let response = emit_response(ty, wrappers, ctx, wire_version)?;
            let item_codec = match wire_version {
                Some(_) => response.wire_codec_expr.clone(),
                None => response.wire_codec_expr.clone(),
            };
            let send_wire_arg = match wire_version {
                Some(version) => format!("{{ tag: 'V{version}' as const, value: item }}"),
                None => "item".to_string(),
            };
            writedoc!(
                out,
                    ...
                    {{
                      ...
                      start(bytes, ctx, port) {{
                        ...
                        const sink: SubscriptionSink<{item_type}> = {{
                          send(item) {{ port.sendReceive({item_codec}.enc({send_wire_arg})); }},
                          interrupt() {{ /* untyped interrupt is not supported for this method */ }},
                          get isClosed() {{ return port.isClosed; }},
                        }};
                        return handlers.{ts_method_name}({request_arg}sink, ctx);
                      }},
                    }},
                ",
                item_type = response.inner_type_ts,
        ...
        (MethodKind::ResultSubscription, ReturnType::ResultSubscription { item, err }) => {
            ...
            let interrupt_wire_arg = match wire_version {
                Some(version) => format!("{{ tag: 'V{version}' as const, value: reason }}"),
                    ...
                    {{
                      ...
                      start(bytes, ctx, port) {{
                        ...
                        const sink: SubscriptionSink<{item_type}, {reason_type}> = {{
                          send(item) {{ port.sendReceive({item_codec}.enc({send_wire_arg})); }},
                          interrupt(reason) {{ port.sendInterrupt({interrupt_codec}.enc({interrupt_wire_arg})); }},
                          get isClosed() {{ return port.isClosed; }},
                        }};
                        return handlers.{ts_method_name}({request_arg}sink, ctx);
                      }},
...
fn host_request_decode_expr(payload: &PayloadEmission, wire_version: Option<u32>) -> String {
    if payload.param_list.is_empty() {
        match wire_version {
            Some(version) => format!(
                "({wire_codec}.dec(bytes) as {{ tag: 'V{version}'; value: undefined }}).value;\n          const request = undefined;",
                wire_codec = payload.wire_codec_expr,
                version = version,
            ),
            None => "        // no request params".to_string(),
        }
    } else {
        match wire_version {
            Some(version) => format!(
                "const request = ({wire_codec}.dec(bytes) as {{ tag: 'V{version}'; value: {inner_ts} }}).value;",
                wire_codec = payload.wire_codec_expr,
                version = version,
                inner_ts = payload.inner_type_ts,
            ),
            None => format!(
                "const request = {wire_codec}.dec(bytes) as {inner_ts};",
                wire_codec = payload.wire_codec_expr,
                inner_ts = payload.inner_type_ts,
            ),
        }
