...
fn render(methods: &[(String, MethodEntry)], seen: &BTreeMap<u8, String>) -> Result<String> {
    ...
    writeln!(out, "//! Wire-protocol discriminant table.").unwrap();
    writeln!(out, "//!").unwrap();
    ...
    writeln!(out, "/// A single wire-table row.").unwrap();
    writeln!(out, "pub struct WireEntry {{").unwrap();
    writeln!(out, "    /// Method name from the Rust trait.").unwrap();
    writeln!(out, "    pub method: &'static str,").unwrap();
    writeln!(out, "    /// What kind of slot this entry describes.").unwrap();
    writeln!(out, "    pub kind: WireKind,").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    ...
    .unwrap();
    writeln!(out, "pub enum WireKind {{").unwrap();
    writeln!(out, "    /// Request/response method.").unwrap();
    writeln!(out, "    Request {{").unwrap();
    writeln!(out, "        /// Discriminant for the request frame.").unwrap();
    writeln!(out, "        request_id: u8,").unwrap();
    writeln!(out, "        /// Discriminant for the response frame.").unwrap();
    writeln!(out, "        response_id: u8,").unwrap();
    writeln!(out, "    }},").unwrap();
    writeln!(out, "    /// Subscription method.").unwrap();
    writeln!(out, "    Subscription {{").unwrap();
    writeln!(out, "        /// Discriminant for the start frame.").unwrap();
    writeln!(out, "        start_id: u8,").unwrap();
    writeln!(out, "        /// Discriminant for the stop frame.").unwrap();
    writeln!(out, "        stop_id: u8,").unwrap();
    writeln!(
        out,
        "        /// Discriminant for the interrupt frame (server-initiated termination)."
    )
    .unwrap();
    writeln!(out, "        interrupt_id: u8,").unwrap();
    writeln!(
        out,
        "        /// Discriminant for each receive frame (a streamed item)."
    )
    .unwrap();
    writeln!(out, "        receive_id: u8,").unwrap();
    writeln!(out, "    }},").unwrap();
    writeln!(out, "}}").unwrap();
    writeln!(out).unwrap();
    ...
    .unwrap();
    writeln!(out, "pub const WIRE_TABLE: &[WireEntry] = &[").unwrap();
    for (name, entry) in methods {
        match entry {
            MethodEntry::Request(WireEntry {
                ...
                writeln!(out, "    WireEntry {{").unwrap();
                writeln!(out, "        method: \"{name}\",").unwrap();
                writeln!(out, "        kind: WireKind::Request {{").unwrap();
                writeln!(out, "            request_id: {request_id},").unwrap();
                writeln!(out, "            response_id: {response_id},").unwrap();
                writeln!(out, "        }},").unwrap();
                writeln!(out, "    }},").unwrap();
            }
            MethodEntry::Subscription(SubEntry {
                ...
                writeln!(out, "    WireEntry {{").unwrap();
                writeln!(out, "        method: \"{name}\",").unwrap();
                writeln!(out, "        kind: WireKind::Subscription {{").unwrap();
                writeln!(out, "            start_id: {start_id},").unwrap();
                writeln!(out, "            stop_id: {stop_id},").unwrap();
                writeln!(out, "            interrupt_id: {interrupt_id},").unwrap();
                writeln!(out, "            receive_id: {receive_id},").unwrap();
                writeln!(out, "        }},").unwrap();
                writeln!(out, "    }},").unwrap();
            }
        }
