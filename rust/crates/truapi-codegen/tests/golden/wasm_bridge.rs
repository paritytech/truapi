...
impl truapi_platform::Notifications for WasmPlatform {
	...
	async fn cancel_notification(&self, id: v01::NotificationId) -> Result<(), v01::GenericError> {
		invoke_unit(&self.bridge.cancel_notification, vec![JsValue::from_f64(f64::from(id))])
			.await
			.map_err(generic)
	}
}
...
impl truapi_platform::PreimageHost for WasmPlatform {
	fn lookup_preimage(
		...
		invoke_js_subscription(&self.bridge.lookup_preimage, Some(key), parse_optional_bytes_item)
	}
}
...
impl truapi_platform::ProductStorage for WasmPlatform {
	...
	async fn write(
		...
		invoke_unit(
			&self.bridge.write,
			vec![JsValue::from_str(&key), Uint8Array::from(value.as_slice()).into()],
		)
		.await
		.map_err(|reason| v01::HostLocalStorageReadError::Unknown { reason })
	}
