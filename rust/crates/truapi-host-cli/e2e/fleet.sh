...
#   BOT            signing-bot base URL (default http://localhost:3737)
#   NETWORK        pairing network (default paseo-next-v2)
...
BOT="${BOT:-http://localhost:3737}"
NETWORK="${NETWORK:-paseo-next-v2}"
...
run_vu() {
  local i="$1" port="$2" log
  log="$(mktemp)"
  VU_INDEX="$i" "$BIN" pairing-host --product-id headless-playground.dot \
  ...
  if [ -z "$deeplink" ]; then
    echo "VU$i: no deeplink (host died?)"; tail -3 "$log"; kill "$ph" 2>/dev/null; rm -f "$log"; return 1
  fi
  ...
  wait "$ph"; echo "VU$i: pairing host exit=$?"
  rm -f "$log"
}
