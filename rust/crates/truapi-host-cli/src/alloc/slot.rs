...
/// Scan slots `0..max` for `period`, returning the first free seq (or detecting
/// that `target` already holds one). `entropy` is our bandersnatch entropy.
pub async fn scan_slot(
    rpc: &RpcClient,
    metadata: &Metadata,
    entropy: [u8; 32],
    period: u32,
