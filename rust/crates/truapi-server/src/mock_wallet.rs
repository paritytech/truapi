...
use crate::host_logic::sso::messages::{
    RemoteMessage, RemoteMessageData, SigningPayloadResponseData, SigningResponse, v1,
};
use crate::host_logic::sso::pairing::{
    AES_GCM_NONCE_LEN, SsoStatementData, VersionedHandshakeProposal, VersionedHandshakeResponse,
    encrypt_session_statement_data_with_nonce, v2,
};
...
fn wallet_handshake_statement(deeplink: &str) -> Vec<u8> {
    ...
    let wallet_persistent_public: [u8; 65] = P256SecretKey::from_slice(&[2; 32])
        ...
        .try_into()
        .unwrap();
    let answer = v2::EncryptedResponse::Success(Box::new(v2::Success {
...
impl PreimageHost for MockWalletPlatform {
    async fn submit_preimage(
        &self,
        value: Vec<u8>,
        _bulletin_allowance_signer: truapi_platform::BulletinAllowanceSigner,
    ) -> Result<Vec<u8>, v01::PreimageSubmitError> {
        self.inner
            .submit_preimage(value, _bulletin_allowance_signer)
            .await
    }
    fn lookup_preimage(
        &self,
...
mod tests {
    ...
    // PENDING #104/#264: the signing-response framing drifted with #104's SSO
    // rework (opaque SSO ids / statement-proof routing), so the scripted mock
    // response no longer satisfies the core's sign path. Re-port the signing
    // framing once #104's SSO internals settle (tracked for post-#264 revisit).
    #[ignore = "signing framing pending #104 SSO rework; revisit post-#264"]
    #[test]
