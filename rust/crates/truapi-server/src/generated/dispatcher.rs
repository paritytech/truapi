...
{
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::FUNDING_REQUEST,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::funding::HostFundingRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::funding::HostFundingResponse =
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_subscription(
            wire_table::FUNDING_STATUS_SUBSCRIBE,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::funding::HostFundingStatusSubscribeRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::funding::HostFundingSessionError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Err(encode_versioned_interrupt_payload(
                    ...
                    Ok(subscription_stream::<
                        versioned::funding::HostFundingStatusSubscribeItem,
                        _,
                    >(stream))
                })
            },
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_subscription(
            wire_table::FUNDING_SERVE_SUBSCRIBE,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let _ = bytes;
                    ...
                    let stream = match host.serve_subscribe(&cx).await {
                        ...
                        Err(err) => {
                            return Err(encode_versioned_interrupt_payload(
                                err,
                                <versioned::funding::HostFundingServeError as Versioned>::LATEST,
                            ));
                        }
                    };
                    Ok(subscription_stream::<
                        versioned::funding::HostFundingServeSubscribeItem,
                        _,
                    >(stream))
                })
            },
    ...
    }
    {
        let host = host;
        dispatcher.on_request(
            wire_table::FUNDING_REPORT,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::funding::HostFundingReportRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::funding::HostFundingServingError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::funding::HostFundingReportResponse =
