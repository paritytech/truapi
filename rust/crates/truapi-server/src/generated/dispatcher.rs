...
use truapi::api::{
    Account, Chain, Chat, CoinPayment, Contacts, Entropy, LocalStorage, Notifications, Payment,
...
{
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::CONTACTS_RESOLVE,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::contacts::HostContactsResolveRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::contacts::HostContactsResolveError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::contacts::HostContactsResolveResponse =
                        match host.resolve(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::CONTACTS_SEND,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::contacts::HostContactsSendRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::contacts::HostContactsSendError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
