...
{
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::P_2_P_MEDIA_STATUS,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pStatusRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pStatusError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::p2p_media::HostP2pStatusResponse =
                        match host.status(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::P_2_P_MEDIA_ROOM_CREATE,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pRoomCreateRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pRoomCreateError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::p2p_media::HostP2pRoomCreateResponse =
                        match host.room_create(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::P_2_P_MEDIA_ROOM_JOIN,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pRoomJoinRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pRoomJoinError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::p2p_media::HostP2pRoomJoinResponse =
                        match host.room_join(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::P_2_P_MEDIA_ROOM_LEAVE,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pRoomLeaveRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pRoomLeaveError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::p2p_media::HostP2pRoomLeaveResponse =
                        match host.room_leave(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::P_2_P_MEDIA_PUBLISH,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pPublishRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pPublishError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::p2p_media::HostP2pPublishResponse =
                        match host.publish(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host.clone();
        dispatcher.on_request(
            wire_table::P_2_P_MEDIA_UNPUBLISH,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pUnpublishRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pUnpublishError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Ok(encode_versioned_err_payload(
                    ...
                    let target_version = request.version();
                    let cx = CallContext::with_request_id(request_id.clone());
                    let response: versioned::p2p_media::HostP2pUnpublishResponse =
                        match host.unpublish(&cx, request).await {
                            Ok(value) => value,
                            Err(err) => {
                                return Ok(encode_versioned_err_payload(err, target_version));
                            }
    ...
    }
    {
        let host = host;
        dispatcher.on_subscription(
            wire_table::P_2_P_MEDIA_ROOM_EVENTS,
            move |request_id: String, bytes: Vec<u8>| {
                let host = host.clone();
                Box::pin(async move {
                    let request: versioned::p2p_media::HostP2pRoomEventsRequest =
                        match Decode::decode(&mut &bytes[..]) {
                            Ok(request) => request,
                            Err(err) => {
                                let error: truapi::CallError<
                                    versioned::p2p_media::HostP2pRoomEventsError,
                                > = truapi::CallError::MalformedFrame {
                                    reason: err.to_string(),
                                };
                                return Err(encode_versioned_interrupt_payload(
                    ...
                    Ok(subscription_stream::<
                        versioned::p2p_media::HostP2pRoomEventsItem,
                        _,
                    >(stream))
                })
            },
