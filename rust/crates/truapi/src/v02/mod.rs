...
//! every data type used in their signatures. The two communication patterns are:
//!
...
pub trait TrUApiCalls {
    ...
    /// });
    /// ```
    async fn host_feature_supported(&self, feature: Feature) -> Result<bool, GenericError>;
    ...
    ///
    /// if (!result.success) {
    ///   console.error("Navigation failed:", result.value);
    /// }
    /// ```
    ///
    /// # Host Example
    ...
    /// });
    /// ```
    async fn host_navigate_to(&self, url: String) -> Result<(), NavigateToError>;
...
pub trait Permissions {
    ...
    ///
    /// if (granted.success && granted.value) {
    ///   startCamera();
    /// }
    ///
    ...
    /// if (bioGranted.success && bioGranted.value) {
    ...
    /// ]);
    ///
    /// if (allowed.success && allowed.value) {
...
pub trait LocalStorage {
    ...
    ///
    /// if (!result.success) {
    ///   console.error("Storage write failed:", result.value);
    /// }
    /// ```
    ///
    /// # Host Example
    ...
    /// });
    /// ```
    async fn host_local_storage_clear(&self, key: StorageKey) -> Result<(), StorageError>;
}
...
pub trait AccountManagement {
    ...
    /// });
    /// ```
    async fn host_get_non_product_accounts(&self) -> Result<Vec<Account>, RequestCredentialsError>;
    ...
    async fn host_account_connection_status_subscribe(
        &self,
    ) -> Subscription<AccountConnectionStatus>;
    ...
    /// } else if (result.value.tag === "Rejected") {
    ///   console.log("User declined identity disclosure");
    /// } else if (result.value.tag === "NotConnected") {
    ...
    /// });
    /// ```
    async fn host_get_user_id(&self) -> Result<UserIdentity, UserIdentityError>;
}
...
pub trait Chat {
    ...
    /// });
    /// ```
    async fn host_chat_list_subscribe(&self) -> Subscription<Vec<ChatRoom>>;
    ...
    /// });
    /// ```
    async fn host_chat_action_subscribe(&self) -> Subscription<ReceivedChatAction>;

    /// Subscribes to custom-message render requests issued by the host.
    ///
    /// The product calls this when it is ready to render. Each emitted
    /// [`CustomMessageRenderRequest`] describes a message the host wants
    /// rendered; the product renders it locally.
    ///
    /// # Product Function
    ///
    ...
    async fn product_chat_custom_message_render_subscribe(
        &self,
    ) -> Subscription<CustomMessageRenderRequest>;
}
...
pub trait StatementStore {
    ...
    /// });
    /// ```
    async fn remote_statement_store_submit(&self, encoded: Bytes) -> Result<String, GenericError>;
}
...
pub trait ChainInteraction {
    ...
    /// });
    ///
    /// if (result.success && result.value.tag === "Started") {
...
pub trait Payment {
    ...
    ///
    /// if (!result.success) {
    ///   if (result.value.tag === "InsufficientFunds") {
    ...
    ///   trackPayment(id);
    /// } else if (result.value.tag === "Denied") {
    ///   console.log("User denied the payment");
    /// } else if (result.value.tag === "InsufficientBalance") {
...
pub trait EntropyDerivation {
    ...
    /// });
    /// ```
    async fn host_derive_entropy(&self, key: Vec<u8>) -> Result<Entropy, DeriveEntropyError>;
}
