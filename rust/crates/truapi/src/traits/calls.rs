...
use crate::v02::{GenericError, HandshakeError, NavigateToError};
use crate::versioned::calls::{
    HostFeatureSupportedRequest, HostFeatureSupportedResponse, HostHandshakeRequest,
    HostHandshakeResponse, HostNavigateToRequest, HostNavigateToResponse,
...
pub trait TrUApiCalls: Send + Sync {
    ...
    ) -> Result<HostHandshakeResponse, HandshakeError> {
        let (HostHandshakeRequest::V1(version) | HostHandshakeRequest::V2(version)) = request;
        if version == 1 {
            Ok(HostHandshakeResponse::V2)
        } else {
    ...
    ) -> Result<HostNavigateToResponse, NavigateToError>;
}
