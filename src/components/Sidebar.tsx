...
export default function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Re-expand all groups when version changes
  useEffect(() => {
    ...
    <>
        ...
        <div className="mt-3 relative" ref={versionRef}>
          ...
          >
            <span className="flex items-center gap-1.5">
              <span className={`w-1.5 h-1.5 rounded-full ${version.status === 'stable' ? 'bg-emerald-400' : 'bg-amber-400'}`} />
              ...
              {versions.map(v => (
                <button
                  ...
                  className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-700/50 transition-colors flex items-center justify-between ${
                    v.id === version.id ? 'text-white bg-slate-700/30' : 'text-slate-400'
                  }`}
                >
                  <span className="flex items-center gap-1.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${v.status === 'stable' ? 'bg-emerald-400' : 'bg-amber-400'}`} />
                    {v.label}
                  </span>
                  {v.status === 'stable' && <span className="text-[9px] text-emerald-400 font-medium">STABLE</span>}
                  {v.status === 'preview' && <span className="text-[9px] text-amber-400 font-medium">PREVIEW</span>}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="px-3 pt-3 pb-1">
        <button
          ...
          className={`w-full text-left px-3 py-1.5 rounded-md text-sm transition-all duration-150 ${
            currentPath === versionPrefix + '/' || currentPath === versionPrefix ? 'bg-slate-800 text-white font-medium' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
          }`}
        >
          Overview
        </button>
        <button
          ...
          className={`w-full text-left px-3 py-1.5 rounded-md text-sm transition-all duration-150 flex items-center gap-2 ${
            currentPath === versionPrefix + '/types' || currentPath.startsWith(versionPrefix + '/type/') ? 'bg-slate-800 text-white font-medium' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'
          }`}
        >
          <Database size={14} />
          Data Types
      ...
      <nav className="flex-1 overflow-y-auto px-3 pb-4 pt-1">
        ...
        {groups.map(group => {
          ...
          const hasActive = groupMethods.some(m => currentPath === `${versionPrefix}/method/${m.id}`);
              ...
              {isExpanded && (
                <div className="ml-4 border-l border-slate-700/50 pl-2 animate-slide-down">
                  {groupMethods.map(method => {
                    const isActive = currentPath === `${versionPrefix}/method/${method.id}`;
                    return (
                      <button
                        key={method.id}
                        onClick={() => handleNav(`${versionPrefix}/method/${method.id}`)}
