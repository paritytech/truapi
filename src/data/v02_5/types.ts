...
export const groups: GroupDef[] = [
  ...
  },
  {
    id: 'favourites',
    name: 'Favourites',
    description: 'Read and manage the user\'s bookmarked product catalogue.',
    methods: ['host_favourites_subscribe', 'host_favourites_add', 'host_favourites_forget'],
  },
  {
    id: 'contacts',
    name: 'Contacts',
    description: 'Read the user\'s host-managed address book with context-scoped entries.',
    methods: ['host_contacts_get', 'host_contacts_subscribe'],
  },
];
...
// result2.value === result.value (byte-for-byte identical)`,
  ...
  // Favourites (RFC 0013)
  {
    id: 'host_favourites_subscribe',
    name: 'host_favourites_subscribe',
    group: 'Favourites',
    groupId: 'favourites',
    pattern: 'subscription',
    description: 'Subscribes to the user\'s bookmarked product catalogue. Delivers the full list on each callback. Requires DevicePermission::Favourites. V0.2.5 (RFC 0013).',
    productFunction: 'truApi.favouritesSubscribe(callback)',
    hostHandler: 'container.handleFavouritesSubscribe(handler)',
    request: '_void',
    response: 'Vec<FavouriteProduct>',
    errorType: 'FavouritesErr',
    productExample: `const sub = truApi.favouritesSubscribe((products) => {
  for (const p of products) {
    console.log(p.product_id, p.installed ? "installed" : "not installed");
  }
});`,
    hostExample: `container.handleFavouritesSubscribe((send, interrupt) => {
  send(catalogue.getAll());
  const unsub = catalogue.onChange(() => send(catalogue.getAll()));
  return () => unsub();
});`,
  },
  {
    id: 'host_favourites_add',
    name: 'host_favourites_add',
    group: 'Favourites',
    groupId: 'favourites',
    pattern: 'request-response',
    description: 'Adds or upserts a product to the user\'s bookmarks. Returns the resulting FavouriteProduct record. V0.2.5 (RFC 0013).',
    productFunction: 'truApi.favouritesAdd(productId)',
    hostHandler: 'container.handleFavouritesAdd(handler)',
    request: 'DotNsIdentifier',
    response: 'Result<FavouriteProduct, FavouritesErr>',
    errorType: 'FavouritesErr',
    productExample: `const record = await truApi.favouritesAdd("cool-app.dot");
console.log("Bookmarked at:", record.created_at);`,
    hostExample: `container.handleFavouritesAdd((productId) => {
  return catalogue.upsert(productId, { source: 'remote' });
});`,
  },
  {
    id: 'host_favourites_forget',
    name: 'host_favourites_forget',
    group: 'Favourites',
    groupId: 'favourites',
    pattern: 'request-response',
    description: 'Removes a product from the user\'s bookmarks. V0.2.5 (RFC 0013).',
    productFunction: 'truApi.favouritesForget(productId)',
    hostHandler: 'container.handleFavouritesForget(handler)',
    request: 'DotNsIdentifier',
    response: 'Result<void, FavouritesErr>',
    errorType: 'FavouritesErr',
    productExample: `await truApi.favouritesForget("old-app.dot");`,
    hostExample: `container.handleFavouritesForget((productId) => {
  ...
  // Contacts (RFC 0014)
  {
    id: 'host_contacts_get',
    name: 'host_contacts_get',
    group: 'Contacts',
    groupId: 'contacts',
    pattern: 'request-response',
    description: 'Returns the user\'s contact list (one-shot). Tier 1 by default — entries filtered to the calling product\'s context. Requires DevicePermission::Contacts or ContactsCrossContext. V0.2.5 (RFC 0014).',
    productFunction: 'truApi.contactsGet()',
    hostHandler: 'container.handleContactsGet(handler)',
    request: '_void',
    response: 'Result<Vec<Contact>, ContactsErr>',
    errorType: 'ContactsErr',
    productExample: `const contacts = await truApi.contactsGet();
for (const c of contacts) {
  console.log(c.local.display_name, c.entries);
}`,
    hostExample: `container.handleContactsGet(() => {
  return addressBook.getFiltered(callingProduct);
});`,
  },
  {
    id: 'host_contacts_subscribe',
    name: 'host_contacts_subscribe',
    group: 'Contacts',
    groupId: 'contacts',
    pattern: 'subscription',
    description: 'Subscribes to the user\'s contact list. Delivers the full filtered list on each callback. V0.2.5 (RFC 0014).',
    productFunction: 'truApi.contactsSubscribe(callback)',
    hostHandler: 'container.handleContactsSubscribe(handler)',
    request: '_void',
    response: 'Vec<Contact>',
    errorType: 'ContactsErr',
    productExample: `const sub = truApi.contactsSubscribe((contacts) => {
  updateContactList(contacts);
});`,
    hostExample: `container.handleContactsSubscribe((send, interrupt) => {
  send(addressBook.getFiltered(callingProduct));
  const unsub = addressBook.onChange(() => {
    send(addressBook.getFiltered(callingProduct));
  });
  return () => unsub();
});`,
...
export const dataTypes: DataType[] = [
  ...
  // Permission Types
  {
    ...
    definition: "Status('Notifications', 'Camera', 'Microphone', 'Bluetooth', 'Nfc', 'Location', 'Clipboard', 'OpenUrl', 'Biometrics', 'Favourites', 'Contacts', 'ContactsCrossContext')",
    description: 'Device capability to request access to. V0.2.5: extended to 12 variants adding Favourites (RFC 0013), Contacts and ContactsCrossContext (RFC 0014).',
    variants: [
      { name: 'Notifications', type: '_void', description: 'Push notification permission' },
      { name: 'Camera', type: '_void', description: 'Camera access' },
      { name: 'Microphone', type: '_void', description: 'Microphone access' },
      { name: 'Bluetooth', type: '_void', description: 'Bluetooth access' },
      { name: 'Nfc', type: '_void', description: 'NFC access' },
      { name: 'Location', type: '_void', description: 'Location access' },
      { name: 'Clipboard', type: '_void', description: 'Clipboard access' },
      { name: 'OpenUrl', type: '_void', description: 'URL opening permission' },
      { name: 'Biometrics', type: '_void', description: 'Biometric authentication' },
      { name: 'Favourites', type: '_void', description: 'Access to the user\'s bookmarked product catalogue (RFC 0013)' },
      { name: 'Contacts', type: '_void', description: 'Own-context contact list access (RFC 0014)' },
      { name: 'ContactsCrossContext', type: '_void', description: 'Cross-context contact list access — implies Contacts (RFC 0014)' },
    ],
  },
  {
  ...
  // Favourites Types (v0.3, RFC 0013)
  {
    id: 'ProductSource', name: 'ProductSource', category: 'Favourites', source: 'favourites.ts',
    definition: 'Enum({ Remote, Local })',
    description: 'Source from which a product was added to the catalogue.',
    variants: [
      { name: 'Remote', type: '_void', description: 'Discovered via on-chain registry' },
      { name: 'Local', type: '_void', description: 'Sideloaded or manually added' },
    ],
  },
  {
    id: 'FavouriteProduct', name: 'FavouriteProduct', category: 'Favourites', source: 'favourites.ts',
    definition: 'Struct({ product_id, installed, source, created_at, updated_at })',
    description: 'A bookmarked product in the user\'s local catalogue.',
    fields: [
      { name: 'product_id', type: 'DotNsIdentifier', description: 'The product\'s DotNS identifier' },
      { name: 'installed', type: 'bool', description: 'Whether the product is currently installed' },
      { name: 'source', type: 'ProductSource', description: 'How the product was added' },
      { name: 'created_at', type: 'u64', description: 'Unix timestamp when first bookmarked' },
      { name: 'updated_at', type: 'u64', description: 'Unix timestamp of last update' },
    ],
  },
  {
    id: 'FavouritesErr', name: 'FavouritesErr', category: 'Favourites', source: 'favourites.ts',
    definition: 'ErrEnum { NotConnected, Rejected, Unknown({ reason: str }) }',
    description: 'Error from favourites API methods.',
    variants: [
      { name: 'NotConnected', type: '_void', description: 'User is not authenticated' },
      { name: 'Rejected', type: '_void', description: 'User denied permission' },
      { name: 'Unknown', type: 'Struct({ reason: str })', description: 'Catch-all' },
    ],
  },
  // Contacts Types (v0.3, RFC 0014)
  {
    id: 'ContextContactInfo', name: 'ContextContactInfo', category: 'Contacts', source: 'contacts.ts',
    definition: 'Struct({ alias?, account_id? })',
    description: 'Per-context contact information (alias and/or account).',
    fields: [
      { name: 'alias', type: 'Option<Vec<u8>>', description: 'Ring VRF alias in this context' },
      { name: 'account_id', type: 'Option<AccountId>', description: 'Account ID in this context' },
    ],
  },
  {
    id: 'LocalContactInfo', name: 'LocalContactInfo', category: 'Contacts', source: 'contacts.ts',
    definition: 'Struct({ display_name? })',
    description: 'Local (host-side) metadata for a contact.',
    fields: [
      { name: 'display_name', type: 'Option<str>', description: 'User-assigned display name' },
    ],
  },
  {
    id: 'Contact', name: 'Contact', category: 'Contacts', source: 'contacts.ts',
    definition: 'Struct({ local, entries })',
    description: 'A contact from the user\'s address book. Tier 1 access filters entries to the calling product\'s context; tier 2 returns all entries.',
    fields: [
      { name: 'local', type: 'LocalContactInfo', description: 'Host-local metadata' },
      { name: 'entries', type: 'Vec<(ContactContext, ContextContactInfo)>', description: 'Per-context entries keyed by ProductAccountId' },
    ],
  },
  {
    id: 'ContactsErr', name: 'ContactsErr', category: 'Contacts', source: 'contacts.ts',
    definition: 'ErrEnum { NotConnected, Rejected, Unknown({ reason: str }) }',
    description: 'Error from contacts API methods.',
    variants: [
      { name: 'NotConnected', type: '_void', description: 'User is not authenticated' },
      { name: 'Rejected', type: '_void', description: 'User denied permission' },
      { name: 'Unknown', type: 'Struct({ reason: str })', description: 'Catch-all' },
