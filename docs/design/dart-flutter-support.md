...
Client first; **host (dispatcher) is a later, optional phase** (§7, Phase 6) — Flutter products are
overwhelmingly clients of a host, not hosts themselves.
...
| **D8** | **Host (dispatcher) target** | **Defer to Phase 6**, client-first. | Flutter products are clients; host is TS/native today. Add only if a Dart host materializes. |
...
**Client path complete and verified.** Phases 0–5 done (client; host dispatcher = Phase 6, out of scope for this delivery). Verified locally: `cargo build --workspace` + `cargo test --workspace` green, `cargo +nightly fmt --check` clean; Dart `dart analyze --fatal-infos` clean, `dart format --set-exit-if-changed` clean, `dart test` = **33 tests green**.
...
### Phase 6 — (Optional) Dart host
- [ ] `--dart-host-output` flag + `dart/truapi_host`
- [ ] Generated typed handler interfaces + dispatch table + subscription frame ports
