...
Status: implemented for the web host path; native typed mirrors remain planned.
...
- **Web:** `WasmPlatform` (in `truapi-server/src/wasm.rs`) holds the typed Rust request,
  `.encode()`s it to a `Uint8Array`, and calls the generated JS raw adapter in
  `@parity/truapi-host-wasm`. The adapter SCALE-*decodes* it straight back into the
  typed object the host wanted, using the callback contracts exported from
  `@parity/truapi-host/callbacks`.
...
the host implements them directly. On web, `truapi-codegen` owns the typed callback surface
under `@parity/truapi-host/callbacks` and the internal wasm adapter under
`@parity/truapi-host-wasm`; host code does not hand-maintain a bytes-to-types wrapper. For
native, the same direction means no `Data`/`ByteArray` forwarding and no host-side SCALE
codec once UniFFI mirrors land.
...
This keeps the bytes-to-types adapter generated by `truapi-codegen`. Rejected
...
`HostCallbacks` object directly. `WasmRawCallbacks` is an internal generated type in
`@parity/truapi-host-wasm`, while the public callback DTOs and codecs are exported from
`@parity/truapi-host/callbacks`. Subscriptions
...
### User confirmation review type

The host capability is a single action: show a local confirmation UI for a reviewed core
operation. The platform trait therefore exposes one method,
`confirm_user_action(UserConfirmationReview)`, rather than one method per operation. The enum
keeps each payload strongly typed while giving every host one confirmation callback to implement.

Sign-payload and sign-raw confirmations are each invoked from two runtime sites with different
v01 types (standard product-account requests and legacy-account requests), so those remain nested
enums:
...
The top-level review enum then wraps every confirmation kind:
...
The runtime wraps `inner` in the appropriate variant instead of `inner.encode()`; wasm projects
the single generated `confirmUserAction` callback; native should mirror the same enum over UniFFI.
Transport/backend primitives such as `submit_preimage` stay on their capability traits.
...
1. **TS interface**: the typed `HostCallbacks` interface and callback DTO codecs, emitted to
   `@parity/truapi-host/callbacks`.
2. **TS adapter (web)**: the SCALE-decode adapter, emitted to `@parity/truapi-host-wasm` from
   the trait surface — `.dec` the payload, call the typed handler, encode the result. Consumed
   internally by `createWasmProvider`.
...
| `@parity/truapi-host/callbacks`: generated typed callback contract and callback DTO codecs | PR1 |
| `@parity/truapi-host-wasm`: generated internal raw adapter over `WasmRawCallbacks` | PR1 |
