...
- _Determinism_: The same root account + product + key always yields the same 32 bytes on any conforming host.
- _Isolation_: Different products derive independent entropy from the same root account. One product cannot compute another product's values.
- _No runtime round trips_: After the initial SSO handshake shares the `rootEntropySource`, all derivation is a pure local computation.
...
Requested by Gav: products sometimes need to know _who_ the user is (their primary DotNS identity), not just interact with derived accounts. This is a privilege, since it allows a product to identify the user across contexts, so it requires JIT user approval on first call. More sophisticated user agents may let users select a different name they control.
