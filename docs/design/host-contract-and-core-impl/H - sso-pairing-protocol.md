...
2. **The wallet keeps every signing key and only ever signs on request.** During the handshake it sends
   *public* keys + account ids, never a secret (confirmed in the iOS source). Transaction signing and
...
```
HostHandshakeData::V1 {            # SCALE enum, index 0
    statement_store_public_key: [u8;32]   # core session sr25519 account id
    encryption_public_key:      [u8;65]   # core P-256 pubkey, X9.63 uncompressed
    metadata:                   String    # runtime-configured https URL -> {name, icon} fetched off-band by the wallet
}
deeplink = runtime_config.scheme + "pair?handshake=" + hex(SCALE(HostHandshakeData))
```

The bootstrap topic is keyed BLAKE2b-256 with key = the core's statement-store pubkey. Current dotli web
subscribes with `matchAll: [topic]` and does not filter the handshake response by a separate bootstrap
channel:
...
The wallet scans, decodes `HostHandshakeData`, fetches `metadata` for display, and on user approval
submits one statement to `(topic, channel)`, signed by its sr25519 key:

```
AppHandshakeData::V1 {             # SCALE enum, index 0
    encrypted_message: bytes       # AES-GCM(ephemeral ECDH) of the sensitive answer (below)
    public_key:        [u8;65]     # the wallet's EPHEMERAL P-256 pubkey (X9.63)
}
# encrypted_message decrypts to:
SSOHandshakeAnswerSensitiveData {  # SCALE, all fixed-size raw
    shared_secret_derivation_key: [u8;65]   # wallet's PERSISTENT P-256 pubkey (for the steady-state secret)
    root_user_account_id:         [u8;32]   # wallet root sr25519 pubkey (product-account derivation root)
    identity_account_id:          [u8;32]   # wallet identity sr25519 pubkey
}
```

The core receives the statement (verifying its sr25519 proof), runs ECDH(`core_enc_priv`,
`ephemeral_pubkey`) -> HKDF-SHA256 -> AES-GCM to decrypt, and now knows the wallet's persistent ECDH key
+ both account ids. The current web construction is in [D4](<D - crypto-foundation.md>): ECDH uses the
...
(`core_enc_priv` x `shared_secret_derivation_key`). From it, directional 32-byte session ids and the
channel ids:
...
```
 Web product / Core                 People-chain statement store              Wallet
   ...
   build HostHandshakeData; A1.present(QR) ------------------ scan -------------->| decode; fetch metadata
   subscribe(topic = H(ss_pub, enc_pub++"topic"))                                 | approve
                            <--- submit(topic, channel, AppHandshakeData{enc, ephPub}) ---|
   verify proof; ECDH(enc_priv, ephPub)->decrypt
     -> wallet persistent P256 pub, root_acct, identity_acct
   shared_secret = ECDH(enc_priv, persistent_pub)
   session_id.own/peer = H(shared_secret, "session"++..)
   SessionState{ public_key=root_acct, ss_secret, ... }; persist; broadcast Connected
...
the persistent peer P-256 pubkey; derived `session_id.own/peer`. Usernames are **not** in the handshake;
they come from a separate on-chain People-chain identity lookup keyed by `root_user_account_id`.
