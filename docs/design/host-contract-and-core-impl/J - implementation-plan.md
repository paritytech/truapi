...
| 1 | Runtime construction config | Add `RuntimeConfig` to `PlatformRuntimeHost`, WASM constructors, UniFFI constructors, and JS worker init; dotli passes product id/label, metadata URL, People genesis, deeplink scheme | Unit tests for config validation; worker protocol test proves config reaches WASM |
