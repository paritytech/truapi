...
Every flow this RFC describes involves **four actors**: two products (the calling SPAs) and the TrUAPI host running in each product's user agent. A host owns its user's keys, chain connection, scoped holdings, and durable records. Hosts have no private wire to each other — they coordinate only through shared infrastructure already accessible from TrUAPI (the chain itself, the chain's gossip-based statement store, and any other transport the product picks for distribution). Products see only opaque blobs; keys, derivation paths, and raw chain material stay inside the host they belong to.
