//! TrUAPI Protocol v0.2.5 — trait and type definitions.
//!
//! This module defines the [`TrUApi`] trait containing all TrUAPI v0.2.5 methods, along with
...
///
/// V0.3: extended with `Favourites`, `Contacts`, and `ContactsCrossContext`
/// per RFC 0013 and RFC 0014.
///
...
pub enum DevicePermission {
    ...
    Biometrics,
    /// Access to the user's bookmarked product catalogue (RFC 0013).
    Favourites,
    /// Own-context contact list access (RFC 0014).
    Contacts,
    /// Cross-context contact list access — implies Contacts (RFC 0014).
...
// ─── Favourites types (v0.2.5 — Favourites API, RFC 0013) ─────────────────────

/// Unix timestamp in seconds.
pub type Timestamp = u64;

/// Source from which a product was added to the catalogue.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ProductSource {
    /// Discovered via on-chain registry.
    Remote,
    /// Sideloaded or manually added.
    Local,
}

/// A bookmarked product in the user's local catalogue.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FavouriteProduct {
    /// The product's DotNS identifier.
    pub product_id: DotNsIdentifier,
    /// Whether the product is currently installed.
    pub installed: bool,
    /// How the product was added.
    pub source: ProductSource,
    /// When the product was first bookmarked.
    pub created_at: Timestamp,
    /// When the record was last updated.
    pub updated_at: Timestamp,
}

/// Errors from favourites API methods.
#[derive(Debug, Clone, PartialEq, Eq)]
...
// ─── Contacts types (v0.2.5 — Contacts API, RFC 0014) ─────────────────────────

/// Context key for contact entries — same as ProductAccountId.
pub type ContactContext = ProductAccountId;

/// Per-context contact information (alias and/or account).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ContextContactInfo {
    /// Ring VRF alias in this context.
    pub alias: Option<Vec<u8>>,
    /// Account ID in this context.
    pub account_id: Option<AccountId>,
}

/// Local (host-side) metadata for a contact.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub struct Contact {
    ...
    /// Per-context entries. Tier 1 access filters to the calling product's
    /// context; tier 2 returns all entries.
    pub entries: Vec<(ContactContext, ContextContactInfo)>,
}

/// Errors from contacts API methods.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub trait TrUApi {
    ...
    ///
    /// V0.3: subject to JIT account-access permission (RFC 0012).
    fn host_account_get(
        &self,
    ...
    // ── Favourites (v0.2.5, RFC 0013) ─────────────────────────────────────

    /// Subscribes to the user's bookmarked product catalogue. Delivers the
    /// full list on each callback.
    ///
    /// Requires `DevicePermission::Favourites`.
    ///
    /// See [RFC 0013](https://github.com/paritytech/truapi-explorer/pull/16).
    fn host_favourites_subscribe(
        &self,
        callback: Box<dyn FnMut(Vec<FavouriteProduct>) + Send>,
    ) -> Result<Self::Subscription, FavouritesError>;

    /// Adds (or upserts) a product to the user's bookmarks.
    ///
    /// See [RFC 0013](https://github.com/paritytech/truapi-explorer/pull/16).
    fn host_favourites_add(
        &self,
        product_id: DotNsIdentifier,
    ) -> Result<FavouriteProduct, FavouritesError>;
    ...
    // ── Contacts (v0.2.5, RFC 0014) ───────────────────────────────────────

    /// Returns the user's contact list (one-shot). Tier 1 by default —
    /// entries filtered to the calling product's context.
    ///
    /// Requires `DevicePermission::Contacts` (tier 1) or
    /// `DevicePermission::ContactsCrossContext` (tier 2).
    ///
    /// See [RFC 0014](https://github.com/paritytech/truapi-explorer/pull/17).
    fn host_contacts_get(&self) -> Result<Vec<Contact>, ContactsError>;

    /// Subscribes to the user's contact list. Delivers the full filtered
    /// list on each callback.
    ///
    /// See [RFC 0014](https://github.com/paritytech/truapi-explorer/pull/17).
    fn host_contacts_subscribe(
        &self,
        callback: Box<dyn FnMut(Vec<Contact>) + Send>,
    ) -> Result<Self::Subscription, ContactsError>;
}
