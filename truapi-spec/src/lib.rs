...
pub trait TrUApi {
    ...
    fn feature_supported(&self, feature: Feature) -> Result<bool, GenericError>;

    /// Requests the host to open a URL, typically in a new browser tab.
    fn navigate_to(&self, url: String) -> Result<(), NavigateToError>;

    /// Sends a push notification to the user via the host.
    fn push_notification(&self, notification: PushNotification) -> Result<(), GenericError>;
    ...
    fn local_storage_read(&self, key: StorageKey) -> Result<Option<StorageValue>, StorageError>;

    /// Writes a value to the scoped key-value store.
    fn local_storage_write(&self, key: StorageKey, value: StorageValue)
        -> Result<(), StorageError>;

    /// Clears a value from the scoped key-value store.
    fn local_storage_clear(&self, key: StorageKey) -> Result<(), StorageError>;
    ...
    fn get_non_product_accounts(&self) -> Result<Vec<Account>, RequestCredentialsError>;
    ...
    fn account_connection_status_subscribe(
        &self,
    ...
    fn sign_payload(&self, payload: SigningPayload) -> Result<SigningResult, SigningError>;

    /// Requests the host to sign a raw message (not a transaction).
    fn sign_raw(&self, payload: SigningRawPayload) -> Result<SigningResult, SigningError>;
    ...
    fn register_custom_message_renderer(
        &self,
    ...
    /// account's key.
    fn statement_store_create_proof(
        &self,
        product_account_id: ProductAccountId,
        statement: Statement,
    ...
    fn chain_head_unpin(&self, request: ChainHeadUnpinRequest) -> Result<(), GenericError>;

    /// Continues a paused operation (when
    /// [`ChainHeadEvent::OperationWaitingForContinue`] is received).
    fn chain_head_continue(&self, request: ChainHeadOperationRequest) -> Result<(), GenericError>;

    /// Stops an in-progress operation.
    fn chain_head_stop_operation(
        &self,
        request: ChainHeadOperationRequest,
    ) -> Result<(), GenericError>;

    /// Gets the genesis hash for a chain.
    fn chain_spec_genesis_hash(&self, genesis_hash: GenesisHash) -> Result<Hex, GenericError>;

    /// Gets the chain name.
    fn chain_spec_chain_name(&self, genesis_hash: GenesisHash) -> Result<String, GenericError>;

    /// Gets the chain properties as a JSON-encoded string.
    fn chain_spec_properties(&self, genesis_hash: GenesisHash) -> Result<String, GenericError>;

    /// Broadcasts a signed transaction to the network.
    /// Returns an operation ID if accepted, `None` if rejected.
    fn chain_transaction_broadcast(
        &self,
        request: ChainTransactionBroadcastRequest,
    ) -> Result<Option<String>, GenericError>;

    /// Stops broadcasting a transaction.
    fn chain_transaction_stop(
        &self,
